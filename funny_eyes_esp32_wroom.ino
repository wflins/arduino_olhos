...
void desenharDormindo() {
  ...
  if (etapaZ >= 1) desenharZ(103, 27, 5);
  if (etapaZ >= 2) desenharZ(111, 17, 6);
  if (etapaZ >= 3) desenharZ(119, 6, 7);
}
...
void setup() {
  ...
  // I2C padrao do ESP32 WROOM-32: SDA 21 / SCL 22.
  Wire.begin(OLED_SDA, OLED_SCL, OLED_I2C_FREQ);

  // Wire ja foi iniciado acima; por isso periphBegin=false.
  if (!display.begin(
  ...
  // Gerador aleatorio nativo do ESP32.
  ...
  Serial.println(F("Funny Eyes ESP32 WROOM-32 iniciado"));
