...
// ADXL345 conforme a ligacao feita no projeto:
// SDA -> D3/GPIO0, SCL -> D4/GPIO2, CS -> 3V3, SDO -> GND.
#define ADXL_SDA D3
#define ADXL_SCL D4
...
const unsigned long ORIENTATION_STABLE_MS = 600UL;
const int16_t ORIENTATION_THRESHOLD = 140; // ~0,55 g em full-resolution

// QR pre-gerado: WIFI:T:WPA;S:ClimaBox-Setup;P:climabox;;
...
// Estado do ADXL345. Mantemos apenas landscape (rotacoes 1 e 3),
// pois todo o layout do ClimaBox foi desenhado para 160x128.
bool adxlOk=false;
...
void rodape(const String& s){ tft.fillRect(0,116,160,12,ST77XX_BLACK); textoLinha(119,"RST: "+s,tft.color565(100,100,100)); }

// -------------------- ADXL345 --------------------
bool adxlWrite(uint8_t reg,uint8_t value){
  Wire.beginTransmission(ADXL_ADDR);Wire.write(reg);Wire.write(value);return Wire.endTransmission()==0;
}

bool adxlReadBytes(uint8_t reg,uint8_t* buf,uint8_t n){
  Wire.beginTransmission(ADXL_ADDR);Wire.write(reg);if(Wire.endTransmission(false)!=0)return false;
  uint8_t got=Wire.requestFrom((uint8_t)ADXL_ADDR,n);if(got!=n)return false;
  for(uint8_t i=0;i<n;i++)buf[i]=Wire.read();return true;
}

bool iniciarADXL345(){
  Wire.begin(ADXL_SDA,ADXL_SCL);
  Wire.setClock(100000); // conservador por causa dos pinos de boot D3/D4
  delay(10);
  uint8_t id=0;if(!adxlReadBytes(0x00,&id,1)||id!=0xE5)return false;
  adxlWrite(0x2D,0x00); // standby antes de configurar
  adxlWrite(0x31,0x08); // FULL_RES, +/-2g
...
bool lerADXL345(){
  uint8_t b[6];if(!adxlReadBytes(0x32,b,6))return false;
  accelX=(int16_t)((uint16_t)b[1]<<8|b[0]);
  accelY=(int16_t)((uint16_t)b[3]<<8|b[2]);
  accelZ=(int16_t)((uint16_t)b[5]<<8|b[4]);
  return true;
}

void prepararModo();

void aplicarRotacao(uint8_t nova){
  if(nova==rotacaoAtual)return;
  rotacaoAtual=nova;
...
void atualizarOrientacao(){
  ...
  // Com o modulo montado alinhado ao display, Y indica se a tela landscape
  // esta normal ou de cabeca para baixo. Uma zona morta evita trocas ao inclinar.
  uint8_t desejada=rotacaoAtual;
  if(accelY>ORIENTATION_THRESHOLD)desejada=1;
  else if(accelY<-ORIENTATION_THRESHOLD)desejada=3;
  else return;
...
void desenharQrWifi(){ const int e=3,q=2,ox=2,oy=14,t=(29+q*2)*e;tft.fillRect(ox,oy,t,t,ST77XX_WHITE);for(int y=0;y<29;y++)for(int x=0;x<29;x++)if(QR_WIFI[y][x]=='1')tft.fillRect(ox+(x+q)*e,oy+(y+q)*e,e,e,ST77XX_BLACK); }
void mostrarPortalNaTela(){ telaLimpa();desenharQrWifi();textoLinha(16,"CONFIG",ST77XX_YELLOW);tft.setCursor(104,16);tft.print("CONFIG");tft.setCursor(104,34);tft.setTextColor(ST77XX_CYAN);tft.print("ClimaBox");tft.setCursor(104,45);tft.print("-Setup");tft.setCursor(104,65);tft.setTextColor(ST77XX_YELLOW);tft.print("Senha");tft.setCursor(104,78);tft.setTextColor(ST77XX_WHITE);tft.print("climabox");tft.setCursor(104,100);tft.print("192.168");tft.setCursor(104,111);tft.print(".4.1"); }
...
bool conectarWifi(bool esperar){
  WiFi.mode(WIFI_STA);WiFi.setAutoReconnect(true);WiFi.persistent(true);
  if(!WiFi.SSID().length())return abrirPortalConfiguracao();
  WiFi.begin();if(!esperar)return true;
  unsigned long ini=millis();while(WiFi.status()!=WL_CONNECTED && millis()-ini<WIFI_CONNECT_TIMEOUT){delay(100);yield();}
...
void desenharTelaClima(){telaLimpa();String local=cidade+(uf.length()?"-"+uf:"");if(local.length()>17)local=local.substring(0,17);textoLinha(3,local,ST77XX_CYAN,2);desenharIconeClima(weatherCode,27,48);tft.setTextColor(ST77XX_YELLOW);tft.setTextSize(4);tft.setCursor(54,34);if(dadosValidos){tft.print(temperatura,0);tft.print("C");}else tft.print("--C");textoLinha(76,dadosValidos?descricaoTempo(weatherCode):"Aguardando Wi-Fi...");tft.setCursor(5,91);tft.print("Sens: ");if(dadosValidos)tft.print(sensacao,0);else tft.print("--");tft.print("C Umid: ");if(dadosValidos)tft.print(umidade,0);else tft.print("--");tft.print("%");tft.setCursor(5,106);tft.print("Vento: ");if(dadosValidos)tft.print(vento,0);else tft.print("--");tft.print(" km/h");rodape("Pac-Man");}
...
bool buscarClima(){
  ...
  BearSSL::WiFiClientSecure client;client.setInsecure();client.setBufferSizes(2048,512);
  HTTPClient http;String url="https://api.open-meteo.com/v1/forecast?latitude="+String(latitude,5)+"&longitude="+String(longitude,5)+"&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code&timezone=auto";
  if(!http.begin(client,url))return false;http.setTimeout(10000);http.useHTTP10(true);int code=http.GET();if(code!=HTTP_CODE_OK){http.end();return false;}
...
// Canvas reduzido: 160x40 = 12,8 KB em vez de 20,5 KB.
GFXcanvas16 cena(160,40);
uint32_t frameJogo=0;unsigned long inicioCena=0;
void tituloJogo(const String& t,uint16_t c,const String& prox){telaLimpa();textoLinha(5,t,c);rodape(prox);}
...
void setup(){
  ...
  adxlOk=iniciarADXL345();
  Serial.println(adxlOk?"ADXL345 OK":"ADXL345 nao encontrado - orientacao automatica desativada");
  // No clima, esperamos Wi-Fi para garantir a primeira previsao. Nos jogos, conecta em segundo plano.
  bool ok=conectarWifi(modoAtual==MODO_CLIMA);
  if(modoAtual==MODO_CLIMA && ok)buscarClima();
  prepararModo();
}
